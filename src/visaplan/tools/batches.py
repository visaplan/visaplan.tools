...
def batch_tuples(seq, batch_size, **kwargs):
    ...
    >>> list(batch_tuples(seq, 3))             # doctest: +NORMALIZE_WHITESPACE
    [([11, 22, 33], 'items 1 to 3 (batch 1 of 3)'),
     ([44, 55, 66], 'items 4 to 6 (batch 2 of 3)'),
     ([77, 88, 99], 'items 7 to 9 (batch 3 of 3)')]

    ... unless you don't want it to (because the length detection might be an
    expensive operation):
    >>> list(batch_tuples(seq, 3, get_length=False))
    ...                                        # doctest: +NORMALIZE_WHITESPACE
    [([11, 22, 33], 'items 1 to 3 (batch 1)'),
     ([44, 55, 66], 'items 4 to 6 (batch 2)'),
     ([77, 88, 99], 'items 7 to 9 (batch 3)')]

    Of course the last batch is shorter in most cases:
    >>> list(batch_tuples(seq, 5))             # doctest: +NORMALIZE_WHITESPACE
    [([11, 22, 33, 44, 55], 'items 1 to 5 (batch 1 of 2)'),
     ([66, 77, 88, 99],     'items 6 to 9 (batch 2 of 2)')]

    ... but sometimes it will just fit:
    >>> list(batch_tuples(seq[:-1], 4))        # doctest: +NORMALIZE_WHITESPACE
    [([11, 22, 33, 44], 'items 1 to 4 (batch 1 of 2)'),
     ([55, 66, 77, 88], 'items 5 to 8 (batch 2 of 2)')]
    ...
    mask_knownlength -- text mask for sequences with known length, containing
